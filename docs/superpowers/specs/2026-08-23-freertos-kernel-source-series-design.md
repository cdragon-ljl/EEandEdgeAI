...
系列规划为 8 个阶段、12 篇长文。规划文件本身使用 `draft: true`，正文使用连续的 `order: 1` 到 `order: 12`。
...
每篇通常为 800 到 1500 行，内容完整性优先于机械行数。文章使用 6 到 9 个二级标题，避免把函数逐个拆成互不相连的小节。
...
- 12 篇正文和 1 个 draft framework 文件数量准确；
