...
在保留现有 22 篇文章公开 URL 的前提下，重构 PCIe 系列的讲解顺序、跨篇衔接和示例组织，使读者能够从一个可观察现象出发，逐步建立 PCIe 接口与 Linux PCI 子系统的完整认知模型。
...
重构后的系列以 PCIe 通用机制为主角，以 Linux 6.12 PCI 子系统为实现基线。RTL8822CE、RTL8821CE、NVMe、RK356x Root Complex 和 `pci_epf_test` 只在适合的知识点中充当局部案例。

## 2. 非目标

- 不把 22 篇文章改写成 RTL8822CE 或 `rtw88` 专题。
- 不复制野火文章的正文、图示或实验源码。
...
- 使用 Linux 6.12 `rtw88` 或适合的公开主线驱动作为案例。
...
- 保留 Bus Model、Driver Framework、Debug Evidence、Interview/System Design 四个主题。
- 所有比较必须建立在前 18 篇已经解释过的 PCIe 机制上。
- 用同一问题维度比较 USB 和 PCIe，不重复各自系列正文。
...
- 22 篇公开 URL 不变，Frontmatter、导航和交叉链接有效。
- 从 01 到 18 形成连续的 PCIe/Linux 学习路径。
- 19～22 只做跨总线综合，不承担补讲 PCIe 基础的职责。
...
5. 15～18：RC/EP、Endpoint、高吞吐和系统调试。
6. 19～22：USB/PCIe 综合与全系列交叉链接。
