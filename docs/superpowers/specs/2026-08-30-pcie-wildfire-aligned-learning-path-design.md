...
- 所有非简单代码块必须包含辅助阅读的中文注释，说明关键步骤而不是逐行复述语法。
- Driver 示例注释必须覆盖调用前提、资源获取、状态变化、所有权交接、错误回滚和对称清理。
- DMA/Ring/IRQ 示例注释必须指出 CPU/Device Owner、Barrier 位置、发布点、Completion 与停止顺序。
- Shell 命令块对危险操作、占位符和输出目的使用 `#` 注释，普通只读单命令不强制添加无意义注释。
- 对 Linux 官方源码做删节或添加注释时，代码块标题明确标为“简化注释版”，避免让读者误认为是逐字源码。
- 注释服务于理解，不通过每行一条同义注释增加篇幅。
...
- 非简单代码块具有足够的中文注释，读者无需先熟悉完整驱动上下文也能理解关键状态和清理路径。
