...
- **3A 算法**：AE（自动曝光）、AWB（自动白平衡）、AF（自动对焦）由 Rockchip 的 **rkaiq**（AIQ，AI Quality）库驱动——它像"画质管家"，根据场景自动调曝光、白平衡、对焦，也能手动干预（本系列后续有专门篇章讲 3A 原理与调优）
...
为什么必须硬件编码？上一篇的账：1080p30 裸流约 93MB/s，即使降到 8Mbps 码率，软编码也需持续占用 GHz 级 CPU 核。**把编码交给 VENC，CPU 就能专心跑应用与 AI**。配套的 **VDEC** 做硬解码（如回放、双向语音的对端解码）。
...
**目标**：登录板卡，用命令确认"四核 A7、video 节点、I2C 总线、sensor 驱动"这些概念在系统里真实存在。全程只读不改，安全。
...
本系列坚持"**双轨可复现**"：板端（RV1126）跑真实硬件链路，PC 端用 FFmpeg/GStreamer 跑同概念。两者的对应关系：
...
**先在 PC 上把概念跑通，再上板验证**，能省掉大量"硬件还没点亮就怀疑算法"的排查时间。这一条原则贯穿整个系列。
