...
调试先把模糊现象改写成可验证问题，再判断它属于硬件、电源/时钟、设备描述、Driver Core、probe、数据路径还是用户接口层次。随后提出互斥假设，用最小观测证伪：dynamic_debug 打开局部日志，tracepoint 记录稳定事件，ftrace 还原调用/调度时序，再与示波器、逻辑分析仪或总线工具对齐。

本篇处理的是单一故障的定位闭环，不讨论长稳发布门禁（第 28 篇）。目标是保存健康/故障基线，找到第一处偏离，只改变一个变量，并让修复通过正常、边界和恢复回归。
...
- [Using the tracer for debugging](https://docs.kernel.org/trace/ftrace.html)
- [Tracepoints](https://docs.kernel.org/trace/tracepoints.html)

## 六、小结

可靠调试是“问题定义 -> 层次定位 -> 互斥假设 -> 最小证据 -> 软硬件对齐 -> 回归”的循环。工具只为假设服务；同时修改 DTS、延时、时钟和代码即使让现象消失，也无法形成可交接的根因结论。
