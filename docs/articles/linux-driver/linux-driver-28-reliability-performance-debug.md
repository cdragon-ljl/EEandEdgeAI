---
title: "嵌入式知识体系 · Linux 驱动开发实战 #28 · 长稳、性能、故障注入与发布门禁"
...
可靠性工作从可重复描述现象开始，再建立未修改系统的 baseline。遇到偏差后先用 tracepoint 记录子系统事件，再按问题范围选择 ftrace 分析内核时序或 perf 分析采样热点；需要验证错误路径时才进行受控 fault injection；watchdog 只负责在系统失去服务时提供最后恢复，全部结果最终进入自动化发布门禁。

“连续跑了一夜没有死机”不具备这些要素。它没有固定负载、环境、软件和硬件版本，也没有定义错误计数、掉帧、内存增长、温度、网络重连与存储一致性的阈值，更无法在下一次修改内核、DTS 或 rootfs 后重复比较。

本章作为整个 Linux 驱动学习路径的收束，以持续采集、处理、网络传输和数据落盘的板端设备为例，把基线、取证、故障验证、长稳与发布判定连成一个工程闭环。第 8 篇已讲单次故障的调试方法，这里只讨论如何把证据转成系统级回归。
...
## 三、用 tracepoint、ftrace 和 perf 获取因果证据
...
- [Linux kernel tracing](https://docs.kernel.org/trace/index.html)
- [ftrace - Function Tracer](https://docs.kernel.org/trace/ftrace.html)
- [Fault injection capabilities infrastructure](https://docs.kernel.org/fault-injection/index.html)
- [The Linux WatchDog Timer Driver Core](https://docs.kernel.org/watchdog/watchdog-kernel-api.html)
...
长稳测试的价值不在持续时间本身，而在固定输入、持续观测、明确阈值和失败后的可回放证据。性能分析与故障注入都应服务于同一条产品关键路径，watchdog 复位也必须保留原因并触发后续诊断，不能把错误简单清零。
