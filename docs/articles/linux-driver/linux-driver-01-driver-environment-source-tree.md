...
在离开配置之前，还要确认当前内核是否允许加载模块。`/proc/config.gz` 可读时直接检查板端快照；否则检查与运行镜像匹配的构建输出 `.config`：

```sh
# 板端：仅在 /proc/config.gz 可读时执行
zcat /proc/config.gz |
  grep -E '^(CONFIG_MODULES=y|# CONFIG_MODULES is not set)$'

# 主机：板端没有导出配置时，路径换成匹配的构建输出
grep -E '^(CONFIG_MODULES=y|# CONFIG_MODULES is not set)$' \
  /path/to/kernel-build/.config
```

看到 `CONFIG_MODULES=y`，说明模块加载能力已经编进内核；看到 `# CONFIG_MODULES is not set`，等价于 `CONFIG_MODULES=n`。后一种内核没有动态模块加载能力，即使主机能够生成 `.ko`，下一篇的 `insmod` 实验也无法进行，需要先用启用 `CONFIG_MODULES` 的配置重新构建并让开发板运行对应镜像。命令没有输出时，先确认检查的是不是完整且匹配的配置文件，不能把空结果当成已启用。
...
`运行内核 release` 来自板端 `uname -r`。源码使用 Git 管理时，可以先记录基线提交和工作区状态：
...
`git status --short` 没有输出时，提交哈希可以标识这棵仓库中受 Git 跟踪的源码内容；一旦有输出，提交哈希就只代表修改前的基线，不能再单独充当源码身份。对已跟踪的改动，可以把相对于该提交的完整差异和状态一起保存到实验记录目录：

```sh
git diff --binary HEAD > source-tree.patch
git status --short > source-tree-status.txt
```

`git diff --binary HEAD` 同时覆盖已暂存和未暂存的受跟踪改动，但不会收录状态中以 `??` 标出的未跟踪文件。如果这些文件参与内核构建，也要把原文件放入同一份实验归档。源码不由 Git 管理，或者无法可靠拆分本地改动时，可以归档实际参与构建的源码快照，并记录归档文件的校验值：

```sh
sha256sum /path/to/kernel-source-snapshot.tar.gz
```

这样记录的源码身份是“基线提交加工作区差异”，或一份可校验的完整源码快照，而不是只有一个可能与实际内容不符的提交号。`配置文件位置` 和 `内核构建输出位置` 则说明模块编译将从哪里取得生成头文件及符号信息；以后出现某个接口未启用或符号版本不一致时，排查会回到这里。
...
到这里，我们还没有编写一行驱动代码，却已经建立了第一个重要结论：模块面向的不是一个抽象的“ARM Linux”，而是某块板上正在运行的具体内核。主机上的源码快照、构建配置、输出目录和交叉编译器共同决定 `.ko` 的身份，目标板的镜像、DTB 和根文件系统决定它最终进入怎样的运行环境。

可以用记录中的信息做一次交叉核对：板端 `uname -r` 与主机 `make -s kernelrelease` 是否一致，匹配配置是否启用了 `CONFIG_MODULES`，源码快照是否包含未提交的实际改动，编译器目标是否符合板卡架构，配置和构建输出是否确实来自同一次内核构建。某一项还没有证据并不可怕，清楚地保留“未确认”比填入一个看似合理的路径更有助于后续实验。
