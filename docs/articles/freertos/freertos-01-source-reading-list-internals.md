...
最后手算一次有序插入，可以检验前面的理解是否连贯。空表记为 E，E 是哨兵，`pxIndex` 也指向 E。依次插入 A=10、B=5、C=10 后，next 方向应为：

```text
E -> B(5) -> A(10) -> C(10) -> E
```

B 排到头部；C 因为等值比较使用 `<=`，位于先插入的 A 之后。第一次遍历从 E 前进到 B，第二次到 A。若此时删除 A，`uxListRemove` 先把 `pxIndex` 回退到 B；下一次遍历从 B 前进到 C，不会跳过 C，也不会继续引用已经删除的 A。
