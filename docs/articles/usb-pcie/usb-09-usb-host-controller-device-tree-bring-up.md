...
在 ARM/RISC-V SoC 上，USB Host 能否工作往往取决于设备树。常见字段包括控制器 compatible、reg、interrupts、clocks、resets、phys、phy-names、dr_mode、vbus-supply 和 status。初学者不要只复制节点，要理解每个字段对应硬件资源。

`reg` 描述控制器寄存器地址；`interrupts` 描述控制器中断；`clocks` 和 `resets` 控制时钟复位；`phys` 连接 USB PHY；`dr_mode` 决定 host、peripheral 或 otg；`vbus-supply` 关系到是否能给外设供电。设备树写错时，表现可能是控制器驱动不 probe、PHY 初始化失败、插设备无反应或枚举不稳定。
