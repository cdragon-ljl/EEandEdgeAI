---
title: "嵌入式知识体系 · #2 · 函数指针与回调机制：嵌入式状态机的灵魂"
description: "假设你正在写一个 UART 驱动，需要支持不同波特率、不同数据位长度、不同校验模式。如果每换一种配置就写一个 `if-else` 分支，代码会膨胀到难以维护。更棘手的是：当接收一帧数据时，有时需要检查起始位，有时需要校验 CRC，有时需要解析应用层协议——这些逻辑在运行时才能确定。"
pubDate: "2026-07-19"
series: "ee-system"
order: 2
tags: ["嵌入式", "嵌入式知识体系"]
