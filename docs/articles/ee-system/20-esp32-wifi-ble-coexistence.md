---
title: "嵌入式知识体系 · #20 · WiFi / BLE 共存：ESP32 无线协议的时分复用"
description: "ESP32 只有 **一根天线，一个射频前端**，但它同时支持 WiFi（2.4G 802.11 b/g/n）和蓝牙（BLE + Classic）。当两个协议都需要占用射频时，冲突不可避免。乐鑫在 ESP-IDF 中实现了一套**共存调度策略（Coexistence Arbitration）**，让两者有序地分时共享天线。"
pubDate: "2026-07-19"
series: "ee-system"
order: 20
tags: ["嵌入式", "嵌入式知识体系"]
